...
function getTrending (n = 50,type='movie') {
  const urls={tv:'https://www.imdb.com/chart/tvmeter',movie:'https://www.imdb.com/chart/moviemeter'}
  return request(urls[type]).then((data) => {
...
function getTrendingGenre (genre, n = 7) {
  return request(`https://www.imdb.com/search/title?genres=${genre}`).then((data) => {
    let trending = []
    let i = 1
    const $ = cheerio.load(data)
    while (i <= n) {
