...
function getSimilarMovieTitle($) {
  return function(index, element){
    const title = $(this).find(".rec-title").text().replace("\n","").replace(/\s\s+/g, " ");
    return title;
  }
}

function getSimilarMoviesById ($) {
  const similarMoviesList = $('.rec_overview').map(getSimilarMovieTitle($));
  return {"More Like This": Array.from(similarMoviesList)};
}
